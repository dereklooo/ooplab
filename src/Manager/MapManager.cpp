    ...
    if (Mario_->GetPosition().x > 0) {
        ...
        for(const auto& [type,monsters] : (*Monsters)) {
            for(const auto &monster : monsters) {
                monster->SetPosition({monster->GetPosition().x - 5.5,monster->GetPosition().y});
            }
        }
        for (const auto& [type,blocks] : (*Blocks)) {
            for(const auto &block : blocks) {
                block->SetPosition({block->GetPosition().x - 5.5 , block->GetPosition().y});
            }
        }
        for(auto &[type,items] : (*Items)) {
            for(const auto &item : items) {
                item->SetPosition({item->GetPosition().x - 5.5 , item->GetPosition().y});
            }
        }
